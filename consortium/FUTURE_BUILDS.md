...
| `bridges.py` | ✅ shipped 2026-04-27 | `FrameReading ↔ Primitive ↔ ClaimNode` connectors. Forward + inverse complete: `reading_to_primitives`, `frame_reading_to_primitives`, `primitives_to_claim_graph`, `primitives_to_typed_claim_graph` (with `CouplingMetadata` + `TypedClaimGraph` per CLAUDE_REQUIREMENTS.md §3), `trajectory_summary`, `trajectory_to_frame_reading`. 94 tests. |
| `router/consent.py` | ✅ shipped 2026-04-27 | `ConsentGate` (in-memory) + `PersistentConsentGate` (JSONL append-only, survives process restarts). 19 tests. |
